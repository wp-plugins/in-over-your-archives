...
function ioya_update_year($current_year = 0, $current_month = 0, $ajax = false) {
	...
	if( empty($posts) ) $current_month = 0;
	...
	while( !$current_month ) {
		...
		if( $month_count < 0 ) $month_count = $current_month + 1;
		if( $month_count > 12 ) break;
	
		$current_month = $results[$month_count];
	
		if( $month_count < $current_month ) $month_count--;
		else $month_count++;
	}

	// These can be generated by the date() function, but you'd have to use mktime which is ugly
	$month_strings = array('', __('JAN', 'ioya'), __('FEB', 'ioya'), __('MAR', 'ioya'), __('APR', 'ioya'), __('MAY', 'ioya'), __('JUN', 'ioya'), __('JUL', 'ioya'), __('AUG', 'ioya'), __('SEP', 'ioya'), __('OCT', 'ioya'), __('NOV', 'ioya'), __('DEC', 'ioya') );
    ...
    <div id="inoveryouryear_<?php echo esc_attr($current_year) ?>" class="inoveryouryear">
        <ul>
          ...
          <li<?php echo $selected; ?>><a href="<?php echo get_month_link($current_year, $month) ?>" rel="<?php echo $current_year . ioya_format_month($month) ?>"><?php echo $month_strings[$month]; ?></a></li>
          <?php else : ?>
          <li<?php echo $selected; ?>><span rel="<?php echo $current_year . ioya_format_month($month) ?>"><?php echo $month_strings[$month]; ?></span></li>
          <?php endif; ?>
          <?php endfor; ?>
        </ul>
      </div>
...
function ioya_update_month($current_year, $current_month) {
	global $wp_query;
 ...
 */
function ioya_archive( $year = 0, $month = 0, $load_scripts = false ) {
	?>
	
	<?php if( $load_scripts ) : ?>
		<?php // Yes, I know the stylesheet below breaks validation, but stop whining :P This is the best way to do it without loading the stylesheet across all pages. Plues, HTML5 is on my side anyway :P ?>
		<link rel="stylesheet" type="text/css" href="<?php echo IOYA_PLUGIN_URL . '/css/in_over_your_css.css' ?>" />
...
function ioya_get_recent_month_with_posts( $current_year, $current_month ) {
	global $wpdb;
...
function ioya_get_custom_img( $post_id ) {
	...
	if ( isset( $custom_thumb ) && $custom_thumb != '' )
		return get_bloginfo('template_url').'/images/thumbs/'.$custom_thumb;
